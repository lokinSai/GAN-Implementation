...
class InfoGANDeep():
    ...
    def _make_generator_model(self):
        ...
        gen = Dense(4*4*1024, use_bias=False, kernel_initializer=init)(in_noise)
        ...
        gen = Conv2DTranspose(512, (5, 5), strides=(1, 1), padding='same',
            use_bias=False, kernel_initializer=init)(gen)
        gen = BatchNormalization()(gen)
        gen = LeakyReLU(alpha=0.1)(gen)

        gen = Conv2DTranspose(256, (5, 5), strides=(2, 2), padding='same',
            use_bias=False, kernel_initializer=init)(gen)
        gen = BatchNormalization()(gen)
        gen = LeakyReLU(alpha=0.1)(gen)

        gen = Conv2DTranspose(128, (5, 5), strides=(2, 2), padding='same',
            use_bias=False, kernel_initializer=init)(gen)
        gen = BatchNormalization()(gen)
        gen = LeakyReLU(alpha=0.1)(gen)
    ...
    def _make_discriminator_model(self):
        ...
        d = Conv2D(512, (5,5), strides=(2,2), padding='same', kernel_initializer=init)(d)
        d = LeakyReLU(alpha=0.1)(d)
        d = Dropout(0.3)(d)

        d = Conv2D(1024, (5,5), strides=(2,2), padding='same', kernel_initializer=init)(d)
        d = LeakyReLU(alpha=0.1)(d)
        d = Dropout(0.3)(d)
